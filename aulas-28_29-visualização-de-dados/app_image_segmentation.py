...
# colors
colors = {'background': '#282b38', 'text': '#a5b1cd'}
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            dcc.Dropdown(id='choose-k', options=[{'label': str(i), 'value': i} for i in range(1, 11)], value=1)],
                 style={'width': '10%',
...
def segment_image(_, image, k: int):
    ...
    raw_fig.update_layout(plot_bgcolor=colors['background'], paper_bgcolor=colors['background'])
    raw_fig.update_traces(hovertemplate=None, hoverinfo='skip')

    # display raw image
    raw_plot = html.Div([html.H3('Raw image: '), dcc.Graph(figure=raw_fig)])
    ...
    seg_fig.update_layout(plot_bgcolor=colors['background'], paper_bgcolor=colors['background'])
    seg_fig.update_traces(hovertemplate=None, hoverinfo='skip')

    # display segmented image
    kmeans_plot = html.Div([html.H3('Segmented image: '), dcc.Graph(figure=seg_fig)])
